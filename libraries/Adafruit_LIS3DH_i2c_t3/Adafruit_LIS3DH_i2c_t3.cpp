/**************************************************************************/
/*!
    @file     Adafruit_LIS3DH_i2c_t3.cpp
...
void Adafruit_LIS3DH::read(void) {
  ...
  if (_cs == -1) {
    ...
    Wire1.write(LIS3DH_REG_OUT_X_L | 0x80); // 0x80 for autoincrement
    Wire1.endTransmission();

    Wire1.requestFrom(_i2caddr, 6);
    x = Wire1.read(); x |= ((uint16_t)Wire1.read()) << 8;
    y = Wire1.read(); y |= ((uint16_t)Wire1.read()) << 8;
    z = Wire1.read(); z |= ((uint16_t)Wire1.read()) << 8;
  } 
  #ifndef __AVR_ATtiny85__
  else {
    if (_sck == -1)
...
int16_t Adafruit_LIS3DH::readADC(uint8_t adc) {
  ...
  if (_cs == -1) {
    // i2c
    Wire1.beginTransmission(_i2caddr);
    Wire1.write(reg | 0x80);   // 0x80 for autoincrement
    Wire1.endTransmission();
    Wire1.requestFrom(_i2caddr, 2);
    value = Wire1.read();  value |= ((uint16_t)Wire1.read()) << 8;
  } 
  #ifndef __AVR_ATtiny85__
  else {
    if (_sck == -1)
